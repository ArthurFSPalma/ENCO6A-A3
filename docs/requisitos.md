...
**Critérios de Aceitação:**
- O sistema deve salvar os dados calculados estruturados em formato de tabela textual ou listagem limpa em um arquivo de texto `.txt`.
...
- **Formato da saída:** definido por HU-01.4 e HU-02.2 — ambos os modos exportam os dados para um arquivo `.txt` diretamente na pasta do projeto. O modo pesquisador usa listagem técnica; o modo jornalista usa a máscara `"Qualidade do ar [Faixa] em [DD/MM/AAAA] na [Estação]"` sem siglas ou valores numéricos de concentração.
...
- **[Resolvido na Sprint 4]** O período de análise não é mais integralmente processado: o sistema requer uma entrada de data específica no formato `DD-MM-YYYY` e exporta o relatório unicamente referente a ela.
